...
public class Level5 : MonoBehaviour
{
    ...
    void Start()
    {
        Result = Random.Range(10, 21);
        Debug.Log("Result" + Result);
        Number1 = Random.Range(1, Result);
    ...
    void Update()
    {
        int child1 = AnswerPanel.transform.childCount;
        int child2 = AnswerPanel2.transform.childCount;
        if (child1 > 0 && child2 > 0)
        {
            string name = AnswerPanel.transform.GetChild(0).name;
            string name2 = AnswerPanel2.transform.GetChild(0).name;
            int? answer = null;
            int? answer2 = null;
            switch (name)
            {
            ...
            string concatAnswer = string.Concat(answer.ToString(), answer2.ToString());
            Debug.Log(concatAnswer);
            CheckResult(int.Parse(concatAnswer));
        }
    }
